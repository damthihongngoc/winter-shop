...
export const addToCartService = async ({ user_id, detail_id, quantity }) => {
  ...
  const [existing] = await pool.query(
    "SELECT * FROM cart_items WHERE cart_id = ? AND detail_id = ?",
    [cartId, detail_id]
  );
  ...
  const [insert] = await pool.query(
    "INSERT INTO cart_items (cart_id, detail_id, quantity) VALUES (?, ?, ?)",
    [cartId, detail_id, quantity]
  );
...
export const getTotalQuantityByUserService = async (userId) => {
  const [result] = await pool.query(
    `
