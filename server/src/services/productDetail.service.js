...
export const getAllProductDetailByIdService = async(product_id) => {
  ...
  `, [product_id]);
    ...
    // ❌ Không có product_detail
    if (mainRows.length === 0) {
        return {
            productDetailMain: null,
            otherProductDetails: [],
            images: []
        };
    }
    ...
    const productDetailMain = {
        ...
        product_thumbnail: mainRows[0].product_thumbnail ?
            URL + mainRows[0].product_thumbnail :
            null,
    };

    const mainDetailId = productDetailMain.detail_id;

    // 2. Lấy các product_detail còn lại
